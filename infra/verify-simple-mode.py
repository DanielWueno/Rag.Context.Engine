...
def load_historical_questions(logs_dir: str, collection: str, response_mode: str) -> list[dict]:
    entries = []
        ...
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                if entry.get("Collection") != collection:
                    continue
                if entry.get("ResponseMode") != response_mode:
                    continue
                entries.append(entry)
    return entries


def ask(base_url: str, entry: dict, timeout: float) -> str | None:
    payload = {
        "query": entry["Query"],
...
def main() -> None:
    ...
    parser.add_argument("--response-mode", default="Simple")
    ...
    entries = load_historical_questions(args.logs_dir, args.collection, args.response_mode)
    if not entries:
        print(
            f"No se encontraron preguntas históricas para collection={args.collection} "
            f"responseMode={args.response_mode} en {args.logs_dir}/rag-api-*.json",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    for i, entry in enumerate(entries, 1):
        query = entry["Query"]
        try:
            answer = ask(args.base_url, entry, args.timeout)
        ...
        if violations:
            violation_count += 1
        print(
            f"[{i}/{len(entries)}] {status} ({len(violations)} violaciones) — {query[:70]!r}",
            file=sys.stderr,
        )

        results.append({"query": query, "answer": answer, "violations": violations})

    answered = len(entries) - error_count
    print("", file=sys.stderr)
    print(
        f"=== Resultado: {answered - violation_count}/{answered} respuestas sin violaciones "
        f"(criterio de la Fase 1: {answered}/{answered}); {error_count} errores de transporte ===",
        file=sys.stderr,
    )
    if violation_count:
        print(
