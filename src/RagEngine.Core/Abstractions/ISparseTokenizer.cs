...
///   The sparse vector format requires uint indices, so the hash is taken
...
/// <summary>
/// Converts raw text into a sparse vector suitable for a named-vector
...
public interface ISparseTokenizer
{
    ...
    ///   sorted ascending by <see cref="SparseEntry.TermIndex"/> (required by the
    ///   vector store's sparse-index format).
